...
class ArticleView implements ViewInterface
{
    ...
    protected function navbarArticle($title = null)
    {
        $title1 = _("Article");
        $list = [
            "/admin/article" => _("View all"),
            "/admin/article/new" => _("Add new")
        ];
        $level = 2;
    ...
    static private function formArticle($case = "new", $value = null, $ID = null)
    {
        $content[] = $case == "edit" ? self::input("id", "hidden", $ID) : null;
        // title
        $content[] = self::fieldsetWithInput(_("Title"), "headline", $value['headline'], ["style" => "width: calc(100% - 60px);"]);
        // position
        $content[] = self::fieldsetWithInput(_("Position"), "position", $value['position'], [ "style" => "width: 40px;" ], "number", [ "min" => "1" ]);
        // article body
        $content[] = [ "tag" => "fieldset", "attributes" => ["style" => "width: 100%;"], "content" => [
            [ "tag" => "legend", "content" => _("Text") ],
            [ "tag" => "textarea", "attributes" => [ "id" => "article_body", "name"=>"articleBody", "style"=>"height: 200px;" ], "content" => stripslashes($value['articleBody']) ]
        ]];
        ...
        $content[] = [ "tag" => "fieldset", "attributes" => ["style" => "width: calc(100% - 660px);"], "content" => [
            ...
            [ "tag" => "input", "attributes" => [ "name" => "articleSection", "type" => "text", "value" => $value['articleSection'] ] ]
        ]];
        // dates
        if ($case == "edit") {
            // date created
        ...
        $content[] = [ "tag" => "fieldset", "attributes" => ["style" => "width: 110px;"], "content" => [
            ...
            [ "tag" => "label", "content" => [
                [ "tag" => "input", "attributes" => [ "name" => "publishied", "type" => "radio", "value" => 1, $value['publishied'] == 1 ? "checked" : null ] ],
                " "._("Yes")
            ]],
            [ "tag" => "label", "content" => [
                [ "tag" => "input", "attributes" => [ "name" => "publishied", "type" => "radio", "value" => 0, $value['publishied'] == 0 ? "checked" : null ] ],
                " "._("No")
            ]]
        ]];
        // autor
